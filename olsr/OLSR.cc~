	...
	if (argc == 2) {
   		 ...
   		 if(strncasecmp(argv[1], "hacker", 6) == 0) {
     			malicious = true;  
                        ...
                        dosnode = OLSR::node_id(ra_addr());
			printf("Attacker node is %d\n",dosnode);

                        FILE *dumpFile3;
			dumpFile3 = fopen("Datafiles/DOSNode.txt", "w");
			fprintf(dumpFile3, "%d\n",dosnode);
        		fclose(dumpFile3);
      			return TCL_OK;
   		 }
			// Added for Blackhole Attack 
			// Added for Attack Victim 
   		   if(strncasecmp(argv[1], "victim", 6) == 0) {
     			malicious = true;  
                        vicnode = OLSR::node_id(ra_addr());
			printf("Victim node is %d\n",vicnode);

                        FILE *dumpFile7;
			dumpFile7 = fopen("Datafiles/VictimNode.txt", "w");
			fprintf(dumpFile7, "%d\n",vicnode);
        		fclose(dumpFile7);
      			return TCL_OK;
   		 }
			// Added for Secure transmission 
 if(strncasecmp(argv[1], "secure", 6) == 0) {
     			secure = true;  
                        vicnode = OLSR::node_id(ra_addr());
			printf("Secure node is %d\n",secnode);

                        FILE *dumpFile8;
			dumpFile8 = fopen("Datafiles/SecureNode.txt", "w");
			fprintf(dumpFile8, "%d\n",secnode);
        		fclose(dumpFile8);
      			return TCL_OK;
   		 }
...
FILE *dumpFile1;
int nodeid = OLSR::node_id(ra_addr());
  dumpFile1 = fopen("Datafiles/MPRSET.txt", "a+");

std::string fname7 = "DOSNode.txt";
std::string fname8;
std::string path2="Datafiles/";
std::stringstream sstm4;
sstm4 << path2 << fname7;
fname8 = sstm4.str();

//dumpFile5 = fopen(fname6.c_str(),"w");
std::fstream myfile2(fname8.c_str());
int dosn1;
int b1;
    while (myfile2 >> b1)
    {   //putchar(b1);
...
std::string fname9 = "VictimNode.txt";
std::string fname10;
std::string path3="Datafiles/";
std::stringstream sstm5;
sstm5 << path3 << fname9;
fname9 = sstm5.str();


std::fstream myfile4(fname9.c_str());
int vicn1;
int b3;
    while (myfile4 >> b3)
    {   //putchar(b3);
...
////////////////////21-11-2017
		{                       
                                   if(secure == true)
				    {
                                     if(nb_tuple->nb_main_addr() == dosn1)
                                        { state_.insert_mpr_addr(dosn1+1);
                                          fprintf(dumpFile1, "\nNODE\tNB\tCurrent Time\n"); 
                                          fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, dosn1+1, CURRENT_TIME);  
                                        }
                                     else
                                       {  state_.insert_mpr_addr(nb_tuple->nb_main_addr());
                                          fprintf(dumpFile1, "\nNODE\tNB\tCurrent Time\n"); 
                                          fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, nb_tuple->nb_main_addr(), CURRENT_TIME);  
                                        }
     				    }
					...
					if(nodeid == vicn1)
                                        {					
    					//state_.insert_mpr_addr(nb_tuple->nb_main_addr());
					state_.insert_mpr_addr(dosn1);
					
					//////////////////////		
                                        fprintf(dumpFile1, "\nNODE\tNB\tCurrent Time\n");
  
 				        //fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, nb_tuple->nb_main_addr(), CURRENT_TIME);
					fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, dosn1, CURRENT_TIME);

					///////////////////////     
					}

					else if(nodeid != dosn1)
                                        {
                                          state_.insert_mpr_addr(nb_tuple->nb_main_addr());
...
//////////////////////////////
	...
	for (nb2hopset_t::iterator it = N2.begin(); it != N2.end(); it++) {
		...
		if (!found) {
			   ...
			   if(secure == true)
				   {
                                     if(nb2hop_tuple1->nb_main_addr() == dosn1)
                                     {
                                        state_.insert_mpr_addr(dosn1+1);
					fprintf(dumpFile1, "\nNODE\tNB\tCurrent Time\n");
                                        fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, dosn1+1, CURRENT_TIME);
					...
					fprintf(dumpFile1, "\nNODE\tNB\tCurrent Time\n");
                                        fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, nb2hop_tuple1->nb_main_addr(), CURRENT_TIME);
                                      }
                                    }
                          else if (malicious == true )
                           {
		    ...
		    if(secure == true)
				   {
					...
					fprintf(dumpFile1, "\nNODE\tNB\tCurrent Time\n");
					fprintf(dumpFile1,"%d \t %d \t %f\n", nodeid, max->nb_main_addr(), CURRENT_TIME);
                                       } 
                                    }
                     else if (malicious == true )
                     {	
...
int nodeid = OLSR::node_id(ra_addr());
  dumpFile = fopen("Datafiles/rtable.txt", "a+");
  
std::string fname5 = "DOSNode.txt";
std::string fname6;
std::string path1="Datafiles/";
std::stringstream sstm3;
sstm3 << path1 << fname5;
fname6 = sstm3.str();

//dumpFile5 = fopen(fname6.c_str(),"w");
std::fstream myfile1(fname6.c_str());
int dosn;
int b;
    while (myfile1 >> b)
    {   //putchar(b);
...
std::string fname11 = "VictimNode.txt";
std::string fname12;
std::string path5="Datafiles/";
std::stringstream sstm7;
sstm7 << path5 << fname11;
fname12 = sstm7.str();


std::fstream myfile5(fname12.c_str());
int vicn2;
int b4;
    while (myfile5 >> b4)
    {   //putchar(b4);
		...
		if (nb_tuple->status() == OLSR_STATUS_SYM) {
			...
			for (linkset_t::iterator it2 = linkset().begin(); it2 != linkset().end(); it2++) {
				...
				if (get_main_addr(link_tuple->nb_iface_addr()) == nb_tuple->nb_main_addr() && link_tuple->time() >= CURRENT_TIME) {
				   ...
				   if(secure == true)
				   {
                                     if(link_tuple->nb_iface_addr() == dosn)
                                      {
                                        rtable_.add_entry(link_tuple->nb_iface_addr(),
							dosn+1,
							link_tuple->local_iface_addr(),
							1); 
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, link_tuple->nb_iface_addr(), dosn+1, link_tuple->local_iface_addr() , 1);
                                     }
                                    else
                                     {
                                         rtable_.add_entry(link_tuple->nb_iface_addr(),
							...
							1); 
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, link_tuple->nb_iface_addr(), link_tuple->nb_iface_addr() , link_tuple->local_iface_addr() , 1);
                                     }
                                   }
                                  else if (malicious == true )
				    {
					if(nodeid == vicn2)
					{
							//rtable_.add_entry(link_tuple->nb_iface_addr(), link_tuple->nb_iface_addr(), link_tuple->local_iface_addr(),1);
rtable_.add_entry(link_tuple->nb_iface_addr(), dosn, link_tuple->local_iface_addr(),1);
						//////////////////////	
							
                                       			 fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
  
 				        			//fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, link_tuple->nb_iface_addr(), link_tuple->nb_iface_addr() , link_tuple->local_iface_addr() , 1);
fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, link_tuple->nb_iface_addr(), dosn , link_tuple->local_iface_addr() , 1);
					}
				...
				if(secure == true)
				   {
						...
						1); 
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb_tuple->nb_main_addr(), dosn+1 , lt->local_iface_addr() , 1);
                                     }
						...
						1); 
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb_tuple->nb_main_addr(), lt->nb_iface_addr() , lt->local_iface_addr() , 1);
                                    }
                                    }
                                  else if (malicious == true )
                                   {					
					if(nodeid == vicn2)
					{	
						//rtable_.add_entry(nb_tuple->nb_main_addr(), lt->nb_iface_addr(), lt->local_iface_addr(),1);
					rtable_.add_entry(nb_tuple->nb_main_addr(), dosn, lt->local_iface_addr(),1);
						//////////////////////////////
						...
						fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
 				        	...
 				        	//fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb_tuple->nb_main_addr(), lt->nb_iface_addr() , lt->local_iface_addr() , 1);
fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb_tuple->nb_main_addr(), dosn , lt->local_iface_addr() , 1);
					}
				  ...
				  if(secure == true)
				   {
					...
					2);     
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb2hop_tuple->nb2hop_addr(), dosn+1 , entry->iface_addr() , 2);
                                     }
                                     else
                                     {
					rtable_.add_entry(nb2hop_tuple->nb2hop_addr(),
					...
					2);     
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb2hop_tuple->nb2hop_addr(), entry->next_addr() , entry->iface_addr() , 2);
                                      }   
                                    }
                                  else if (malicious == true )
				    {	
					if(nodeid == vicn2)
					{
					//rtable_.add_entry(nb2hop_tuple->nb2hop_addr(), entry->next_addr(), entry->iface_addr(), 2);

					rtable_.add_entry(nb2hop_tuple->nb2hop_addr(), dosn, entry->iface_addr(), 2);
						//////////////////////////////
					...
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
 					...
 					//fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb2hop_tuple->nb2hop_addr(), entry->next_addr() , entry->iface_addr() , 2);
fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, nb2hop_tuple->nb2hop_addr(), dosn , entry->iface_addr() , 2);
					}
		...
		for (topologyset_t::iterator it = topologyset().begin();
			...
			if (entry1 == NULL && entry2 != NULL && entry2->dist() == h) {
				   ...
				   if(secure == true)
				   {
						...
						h+1);
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, topology_tuple->dest_addr(), dosn+1 , entry2->iface_addr() , h+1);
                                      }
                                      else
                                      {
                                        rtable_.add_entry(topology_tuple->dest_addr(),
						...
						h+1);
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, topology_tuple->dest_addr(), entry2->next_addr() , entry2->iface_addr() , h+1);
					...
					if(nodeid == vicn2)
					{
					//rtable_.add_entry(topology_tuple->dest_addr(), entry2->next_addr(), entry2->iface_addr(), h+1);
					rtable_.add_entry(topology_tuple->dest_addr(), dosn, entry2->iface_addr(), h+1);
						...
						fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
 						...
 						//fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, topology_tuple->dest_addr(), entry2->next_addr() , entry2->iface_addr() , h+1);
fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, topology_tuple->dest_addr(), dosn , entry2->iface_addr() , h+1);
					}
				 ...
				 if(secure == true)
				   {
						...
						entry1->dist());
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, tuple->iface_addr(), entry1->next_addr() , entry1->iface_addr() , entry1->dist());
                                      }
                                     else
                                      {
                                       rtable_.add_entry(tuple->iface_addr(),
						...
						entry1->dist());
					fprintf(dumpFile, "\nSource\tDestination\tNext\tIface\tDistance\n");
					fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, tuple->iface_addr(), dosn+1, entry1->iface_addr() , entry1->dist());
                                       }
                                     }
                                  else if (malicious == true )
				   {	
					if(nodeid == vicn2)
					{
				           //rtable_.add_entry(tuple->iface_addr(), entry1->next_addr(), entry1->iface_addr(), entry1->dist());
				rtable_.add_entry(tuple->iface_addr(), dosn, entry1->iface_addr(), entry1->dist());
...
fprintf(dumpFile,"%d \t %d \t\t %d \t %d \t %d \n", nodeid, tuple->iface_addr(), dosn , entry1->iface_addr() , entry1->dist());
					}
